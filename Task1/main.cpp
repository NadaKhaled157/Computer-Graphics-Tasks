...
void drawSquare(float x, float y, float size, float r, float g, float b) {
	...
	// Calculate the coordinates for the four corners of the square
	float x1 = x - size / 2;
	float x2 = x + size / 2;
	float y1 = y - size / 2;
	float y2 = y + size / 2;
...
void drawScene()
{
	...
	drawCircle(Scale(32, 34, scaleCircle), 255, 69, 0); // Outer Circle (Orange)
	drawCircle(Scale(32, 34, scaleCircle * 0.89), 0, 0, 0); // Inner Circle (Black)

	float scaleTriangle = 4;
	drawEquilateralTriangle(Scale(12, 30, scaleTriangle), 41, 174, 147);
	drawEquilateralTriangle(Scale(12, 30, scaleTriangle * 0.8), 0, 0, 0); // Color of background

    drawX(57, 34, 20.0f, 5.0f, 0.63f, 0.66f, 0.81f);
    drawSquare(85, 34, 20.0f, 0.84f, 0.63f, 0.98f);
    drawSquare(85, 34, 15.5f, 0, 0, 0);
